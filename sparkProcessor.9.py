...
from tbIntegration import brProbesNumber
...
if __name__ == "__main__":
    ...
    dfBroadcast = dfAll.filter(dfAll.ssid == 'BROADCAST')
    
    
    df = dfBroadcast.agg(approxCountDistinct('ts').alias('count'))

#Contar pnl
    query = df\
        .writeStream\
        .outputMode('complete')\
        .foreach(brProbesNumber)\
        .start()
        # .format('console')\
